...
massive binary black holes \citep[e.g.,][]{vandenheuvel:2017, pavlovskii:2017,
...
through Roche lobe overflow (RLOF) between two non-compact
...
likely decided in the CEB layer \citep[e.g.,][]{ivanova:2013, ivanova:2020}, may be
...
than suggested by observations (e.g., \citealt{wang:2021a}, see also
 ...
 transfer episodes (case BB RLOF, \citealt{laplace:2020}) and tidal
 ...
 fixed $R=100,\ 200,\ 300,\ 500,\ 1000\,R_\odot$.
...
mass coordinate $m$ as \citep[e.g.,][]{dekool:1990, dewi:2000,
  lau:2022}:
\begin{equation}
  \label{eq:BE}
...
the internal energy \citep[solid lines,][]{klencki:2020}. These two cases bracket the range
...
same total mass as the accretor post-RLOF. We chose to compare our
models for the same total mass $M$ because it enters in \Eqref{eq:BE} and
is typically used in rapid population synthesis codes to construct
accretors from single star models \citep[e.g.,][]{hurley:2002}. Nevertheless, it is not obvious that models of the
...
\Figref{fig:toy_models_example}--% \ref{fig:rotation_models_example}, and
...
\begin{figure*}[hbtp]
  ...
  \caption{Binding energy profile at fixed photospheric radius $R$
    ...
    $X<0.1$ (dashed), or $X<0.2$ (dot-dashed). The ggray dashed lines mark
