...
\author{\todo{others: R.~Farmer, K.~Breivik, E.~Zapartas, Y. Gotberg, Matteo,
    B. D. Metzger, M. Lau, M. Cantiello?}}
...
\keywords{stars: massive -- stars: binaries: common envelope -- stars:
binaries: accretors} %% check keywords exist
...
subsequent CE events. \Secref{sec:methods} describe our \mesa\ calculations, and \Secref{sec:intro_res} presents a proof-of-principle
...
stars % \footnote{We turn the stripped donor into a point mass and set
  % the period to infinity on the fly through our
  % \texttt{run\_star\_extras.f90}.},
...
\begin{figure}[btp]
<<<<<<< HEAD
  % TODO: remove hacky path below
  % \includegraphics[width=0.5\textwidth]{../tex/figures/engineered_TAMS.pdf}
=======
>>>>>>> origin/dev
...
\Figref{fig:TAMS_profiles}). We let \mesa\ ``relax'' the TAMS
...
$1000\,R_\odot$.
% where we artificially modify the specific entropy (s), hydrogen (H),
% and helium (He) profiles (we do not change the other elements mass
% fractions) at terminal age main sequence (TAMS, central hydrogen mass
% fraction $X_\mathrm{c}(^1\mathrm{H})<10^{-4}$).
...
mass coordinate $m/M$ as independent coordinate. % , and we
...
\begin{figure}[tp]
  ...
  \caption{Evolution as a function of the photospheric radius $R$ of
    ...
    ratio, which is always smaller than 1, indicating that the
