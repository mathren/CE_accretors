...
\title{Rejuvenated accretors have less bound envelopes:\\ Impact of
  the first Roche lobe overflow on the common envelope in
...
\begin{abstract}
  ...
  transfer may more easily survive subsequent possible CE events
  compared to current predictions. Despite their high mass, our
  accretors also experience extended ``blue loops'' , which may have
  observational consequences for low-metallicity stellar populations
...
\citep[RLOF,][]{kippenhahn:1967}. All of our input files are available
at \url{10.5281/zenodo.6600641} and our output is compatible with the
population synthesis code \code{POSYDON} \citep{fragos:2022} and publicly
available at \todo{zenodo}. Our setup is similar to
\cite{renzo:2021zoph}, except for the metallicity: here we adopt
$Z=0.0019\simeq Z_\odot/10$, relevant for the progenitor population of
GW events \citep[e.g.,][]{vanson:2021}. Moreover, we apply throughout
the star a small amount of mixing with diffusivity
...
e.g.,][]{yoon:2017, gotberg:2017, gotberg:2018, laplace:2020,
  laplace:2021} from ZAMS, through RLOF, until our definition of
...
donor during RLOF. During this phase the accretor is progressively
...
RLOF-accretor triggers a CE. Physically, its outcome is determined by
the thermal and dynamical reaction of the now-shared envelope to the
inspiral \citep[e.g.,][]{ivanova:2013, ivanova:2020, renzo:21gwce}.
However, a common oversimplification is to rely on energy conservation
\citep[$\alpha_{\mathrm{CE}}\lambda_\mathrm{CE}$ algorithms,
e.g.,][]{webbink:1984, dekool:1990} to determine CE ejection and final
separation, therefore, we focus here on the RLOF-accretor/CE-donor binding energy. We
calculate the cumulative binding energy outside outside mass
coordinate $m$ \citep[e.g.,][]{dekool:1990, dewi:2000, lau:2022} as:
\begin{equation}
  \label{eq:BE}
...
\begin{figure*}[htbp]
  ...
  \caption{Ratios of the binding energy profiles (including internal
    ...
    ratio of the accretor to a non-rotating
...
boundary region and ``rejuvenating'' the star. As the accretors evolve
...
RLOF-accretor. Before the onset of the dynamical instability in a CE
...
Accretors have overall lower binding energy of the envelope
(both integrated from the surface to the He core,
see~\Figref{fig:BE_env_R} and as a function of radius,
see~\Figref{fig:BE_profiles}-\ref{fig:grid_ratios}). The
...
% may
imply easier to eject (post-RLOF, second) CE (\Figref{fig:grid_ratios}).

%pop synth application
Including this effect in population synthesis simulations could impact
the distribution of post-CE orbital separations, the predicted number
of ``reverse'' stellar mergers \citep[e.g.,][]{zapartas:2017}, and the
rate of GW mergers. Our grid consists only of three binaries,
but could be extended to inform
semi-analytic approximations of the binding energy of CE-donor that
have accreted mass in a previous stable mass transfer phase (see also
\Figref{fig:lambda_grid}).

For very massive BH progenitors with $M_{\rm ZAMS}\gtrsim 40\,M_\odot$, the
effect discussed here may be less pronounced or absent. First of all,
GW-progenitors of these masses may not experience CE at all
\citep[e.g.,][]{vanson:2021, marchant:2021}. Second, in this regime
the main-sequence lifetime becomes roughly independent of mass at
$\sim{}2.5-3$\,Myr: therefore, at the first RLOF, the accretors may be
already sufficiently evolved to prevent rejuvenation through a steep
core/envelope chemical boundary. However, more massive stars are
generally easier to mix \citep[e.g.,][]{yoon:2005}, therefore the
importance of their structural readjustment to RLOF-accretion for
subsequent binary interactions remains to be studied.
