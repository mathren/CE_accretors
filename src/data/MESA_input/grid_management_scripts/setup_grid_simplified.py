...
def setup_grid_same_core(mass, init_model=None):
    """
    sets up a grid of models of given mass varying the slope of the core-envelope boundary
    ...
    WHERE_TO_RUN = "/mnt/home/mrenzo/ceph/RUNS/accretors/mesa15140/simplified_profiles/same_core/grid"+str(mass)+"/"
    ...
    with open(RUNFILE, "a") as F:
        ...
        if init_model==None:
            init_model = "/mnt/home/mrenzo/ceph/RUNS/accretors/mesa15140/simplified_profiles/TAMS_models/"+str(mass)+"_rot0_to_TAMS/LOGS/TAMS.data"
        ...
        for dm in np.linspace(-4,4,21):
            ...
            if m_out >= min_M_boundary:
                ...
                if (os.path.isdir(DESTINATION)):
                    ...
                    mk_simplified_profile_from_pfile(init_model, outfile=outfile, plot=False, m_in=min_M_boundary, m_out=m_out)
                    chem = outfile+'_composition.txt'
                    entropy = outfile+'_entropy.txt'
                    backline = " && ./clean && ./mk && ./rn 2>&1 | tee out.txt"# +"\n"
                    send_email = " ; tail -n 50 out.txt |  mail -s \""+f"{dm:.2f}"+"\" -r rusty@flatironinstitute.org -S from=\"rusty@flatironinstitute.org\" mrenzo@flatironinstitute.org"
                    ...
                    # modify the inlists
                    os.system('perl -pi.back -e \'s/ENTROPYSTRING/'+str(entropy.split('/')[-1])+'/g;\' inlist_simplified_profile')
                    os.system('perl -pi.back -e \'s/CHEMSTRING/'+str(chem.split('/')[-1])+'/g;\' inlist_simplified_profile')
                    os.system('perl -pi.back -e \'s/MASS/'+str(mass)+'/g;\' inlist_simplified_profile')
