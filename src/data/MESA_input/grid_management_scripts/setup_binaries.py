...
if __name__=="__main__":
    ...
    for Z in metallicities:
        ...
        with open(RUNFILE,"a") as F:
            ...
            header = "#DISBATCH PREFIX export OMP_NUM_THREADS=10 ; export MESA_DIR=/mnt/home/mrenzo/codes/mesa/mesa_15140/mesa15140 ; export MESASDK_ROOT=/mnt/home/mrenzo/codes/mesa/mesa_15140/mesasdk ; source $MESASDK_ROOT/bin/mesasdk_init.sh ;"+"\n"
            F.writelines(header)
                    ...
                    for q in mass_ratios:
                        ...
                        for p in periods:
                            for st_eff in ST_efficiencies:
                                ...
                                # print(p)
                                DESTINATION  = folder_name(m1, m2, p, Z=Z, root=WHERE_TO_RUN_Z)
                                DESTINATION  = DESTINATION+"_"+str(st_eff)+"/"
                                # print(DESTINATION)
                                # now set up stuff
                                backline = " && ./clean && ./mk && ./rn 2>&1 | tee out.txt"# +"\n"
                                convert_history1 = " ; python /mnt/home/mrenzo/codes/python_stuff/plotFunc/convert_to_npy.py "+DESTINATION+"/LOGS1/history.data"
                                convert_history2 = " ; python /mnt/home/mrenzo/codes/python_stuff/plotFunc/convert_to_npy.py "+DESTINATION+"/LOGS2/history.data"
                                convert_bin_history = " ; python /mnt/home/mrenzo/codes/python_stuff/plotFunc/convert_to_npy.py "+DESTINATION+"/binary_history.data"
                                ### leave root empty in line belowfor relative folder name
                                send_email = " ; tail -n 50 out.txt |  mail -s \""+folder_name(m1, m2, p, Z=Z, root="")+"\" -r rusty@flatironinstitute.org -S from=\"rusty@flatironinstitute.org\" mrenzo@flatironinstitute.org"
                                backline = backline+convert_history1+convert_history2+convert_bin_history+send_email+"\n"
                                ...
                                os.system('perl -pi.back -e \'s/PERIOD/'+str(p)+'/g;\' inlist_binary')
                                os.system('perl -pi.back -e \'s/METALLICITY/'+str(Z)+'/g;\' inlist_both')
                                os.system('perl -pi.back -e \'s/METALLICITY/'+str(Z)+'/g;\' inlist_extra')
                                # ST efficiency
                                os.system('perl -pi.back -e \'s/ST_EFFICIENCY/'+str(st_eff)+'/g;\' inlist_both')
                                # resolution
